...
class Blockchain:
    ...
    def add_new_transaction(self, transaction):
        ...
        dstr = json.dumps(self.unconfirmed_transactions);
        nodup = list(self.removeduplicate(json.loads(dstr)));
        d1 = json.dumps(nodup)
        print(d1)
        f = open("valid_json_file.json", "w")
        f.write(d1)
        f.close()
        with open('valid_json_file.json') as f1: 
           for json_obj in f1:
              dobj = json.loads(json_obj) 
              prev = {}
           if (len(dobj) > 0): 
              ch1 = list(self.removeduplicate(dobj));
              pre = ch1[0]
              prev = pre["Aadhar_ID"] 
              print(prev)

        dstr2 = json.dumps(transaction);
        cur = json.loads(dstr2);
        current = cur['Aadhar_ID'];
        print(current) 
    ...
    '''def add_new_transaction(self, transaction):
        ...
        d1 = json.dumps(self.unconfirmed_transactions);
        d3 = json.loads(d1);
        print(len(d3))
        prev = {}
        for aadhar in d3: 
           print(len(aadhar))
           if ((len(d3) > 0) and (len(aadhar)) > 2): 
              if (prev == int(aadhar["Aadhar_ID"])):   
                 self.unconfirmed_transactions.extend(transaction)
              else:  
                 prev = {aadhar["Aadhar_ID"]};
                 print(prev) 
                 self.unconfirmed_transactions.append(transaction)
   '''
    def Convert(self, a):
    ...
    def add_traffic_transaction(self, transaction):
...
def new_transaction():
    ...
    required_fields = ["Aadhar_ID", "name", "address"]
...
@app.route('/traffic_transaction', methods=['POST'])
def traffic_transaction():
    tx_data = request.get_json()
    required_fields = ["Aadhar_ID", "violated_count", "severity"]
    ...
    blockchain.add_traffic_transaction(tx_data)
...
def get_pending_tx():
    ...
    valid_json = json.dumps(blockchain.unconfirmed_transactions)
    return json.dumps(valid_json)

# endpoint to query unconfirmed transactions
@app.route('/merge_tx')
def merge_pending_tx():
   employee_list = {'employee_records':[]}
   with open('valid_json_file.txt') as f1: 
        for json_obj in f1:
   ...
   f.write(valid_json)
   f.close()
   selected_env="Aadhar_ID"
   data = json.loads(valid_json)
   for item in data :
      host_list = [item[selected_env] for item in data if selected_env in item]
      host_list = list(it.chain(*host_list)) # To convert sub-list into flat 
